#!/usr/bin/env python
#ipython --profile=mpi0 ./scripts/oklahoma.py /lcrc/group/earthscience/radar/nexrad_current/KVNX/ nc
import matplotlib
matplotlib.use('agg')
import pyart
...
def do_grid_map_gates_to_grid(radar_fname):
    ...
    import os
    md = '/lcrc/group/earthscience/radar/portland_out/'
    try:
        ...
        if radar.latitude['data'] == 0.0:
            radar.latitude['data'] = np.array([43.8925])
            radar.longitude['data'] = np.array([-70.2572])
        fh = open(tf, 'w')
        fh.write('Calcs \n')
        fh.close()
...
st = '/lcrc/group/earthscience/radar/portland_out/status/'
idir = sys.argv[1]
endswith = sys.argv[2]
